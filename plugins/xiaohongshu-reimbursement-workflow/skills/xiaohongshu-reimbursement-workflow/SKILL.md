...
4. 金额使用十进制定点值，保留用户确认精度且最多三位小数；超过三位小数先确认，禁止二进制浮点汇总和静默舍入。
...
标准总表尚不存在时，只能使用用户明确指定的旧总表作为基线；第二道门禁前不得创建标准名文件。
...
- 稳定交易 ID、日期、人员、项目、汇总标签、金额、类目、是否实报、证据引用和明细行映射；
- 归档副本、工作簿图片锚点、输出路径/SHA256；
- 当前 `reviewRevision` 和业务记录多重集合摘要。语义/视觉验收证书另存于同一任务临时目录，不能写回 manifest 形成自引用。

由同一 manifest 驱动摘要预览、本次明细、截图对应表、归档和候选校验，禁止各阶段重新解析同一材料。验收证书的缓存键必须包含：规则版本、manifest 摘要及其全部依赖文件 SHA256。
...
按批次归档 reference 生成：摘要预览、本次明细、原图归档和带嵌入图片的对应截图表。最终归档不得出现内部 manifest、验收证书、预览、日志或差异说明。

同一构建进程可批量生成所有阶段 B 工作簿；同一语义验证进程各打开一次；同一视觉进程只渲染有界实际数据区。每张来源图只读取、解码和登记一次；业务行不得因图片 SHA256 相同而删除。
...
按候选与发布 reference 绑定基线并生成候选。第二道门禁前不得直接编辑、创建或替换根目录标准总表。候选验收后把它保存为归档内 `小红书支出总表_截至<结束日期>.xlsx`，再次确认根表 SHA256 未变。
...
1. 重新读取本次明细，使用十进制定点金额独立重算目标类目的人员/分类汇总、费用合计和实报合计；独立重算非目标类目小计并确认其未进入目标明细或候选。

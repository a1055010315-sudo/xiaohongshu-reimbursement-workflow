...
- 最终验收基线固定为只读安装版本 `0.5.0+codex.20260819174146`，禁止修改其文件或缓存。使用同一机器、同一完全合成批次和相同冷/热规则，分别比较旧版 `prepare + Gate 1 + finalize` 与新版 `prepare + Gate 1 + full-correspondence Gate 2 finalize` 的插件可控代码阶段中位总耗时；冷、热条件下新版都必须至少快 20%。
...
- 未达到性能门槛时必须进行热点 review，优先消除重复文件读取、图片解码、OOXML 解析和摘要计算；不得通过跳过第二视觉读取、信任 Gate 1 抽取、减少媒体/交易/交付物覆盖或只验哈希来提速。

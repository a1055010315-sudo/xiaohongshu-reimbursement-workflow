...
当用户要求补齐历史遗漏，或修改既有交易的 profile、日期、项目、金额、人员、分类、明细类型、`settlement` 时，继续使用普通父批次的统一门禁和协调发布，但执行本 reference。不要误入只允许零业务变化的 `ledger-reorder-correction`。

本分支的最小事实单位是交易，不是工作表行、费用组合并区或截图。表格排序、D 列公式以及 A/D/E/F 合并均是交易事实的派生布局，不能反向决定交易是否存在、属于谁或属于哪个分类。

## 2. 不变量与来源覆盖

- 新任务使用 manifest v3；恢复既有 v1/v2 批次只读兼容，不把旧 manifest 静默升级为新事实。交易 profile 只允许配置中的三个规范 ID；“住所”规范化为 `residence`。
...
- 每个 profile 的 `company_paid_no_reimbursement` 仍进入该 profile 明细和候选/根表，只从“实报”合计排除。确实不属于三个受控 profile 的材料才可用 `excluded_non_target`。
...
- A 列只按连续相同日期形成运行段。
- D/E/F 使用同一费用组运行段。只有相邻交易的“同 profile + 正式人员/主体 + 精确分类 + 明细行类型 + settlement”均非空且完全相同才成组；日期变化本身不拆组。
- 分类、人员、类型或 `settlement` 任一变化都拆组；任一组键为空时该交易为单行组。
- 多行组的 D/E/F 三列必须使用完全相同的合并范围，D 左上角公式必须精确覆盖本组 C；锚点居中、垂直居中、自动换行且不缩小字体，非锚点 OOXML 为空。行高显式设置，超 7 行或 210pt 的块只预警/预览，不擅自拆分。
- C 的显示精度：整数 0 位；非整数且用户原值为三位小数时 3 位；其余非整数 2 位。D 使用组内最大显示精度，禁止整段统一成三位小数。

每个 profile 候选计划的 SHA256 和 manifest 的 `sourceCoverageDigest` 必须进入父批次统一 Gate 上下文。
...
每次候选变化都重新生成该 profile 的布局计划、来源覆盖摘要、候选预览和父 Gate 1。收到第一道精确确认后，再从磁盘独立终审并生成父 Gate 2；profile 集合、候选计划、来源覆盖、基线、候选或补丁任一变化都使整个父批次两道门禁失效。

收到第二道精确确认后，发布前再次重读所有受影响根表、候选、manifest v3、布局计划和来源覆盖证书；所有摘要一致才调用多目标协调发布器。发布后重新打开每个目标，运行同一独立审计并确认根表 SHA256 等于对应候选 SHA256。任一失败按父事务恢复全部已替换目标。

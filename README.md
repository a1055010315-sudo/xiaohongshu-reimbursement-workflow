...
| `0.5.0+codex.20260823161923` | 稳定回滚版本 | 只包含发放归档启动门槛修复，不包含整体性能优化；tag 与 Release 保持不变，可随时用已核验的 ZIP 回滚。 |
| `0.5.0+codex.20260823170757` | 当前稳定版本 | 在上一稳定版的业务语义和安全链不变的前提下，减少重复构建和内存副本、限制 PDF Worker、复用单次审计内的共享来源，并将候选文件最多三路安全并发写入。最终 A/B 中普通报销冷/热 p50 均快约 7.0%，发放冷/热 p50 分别快约 8.0%/8.7%，输出逐字节等价。成品文件名为 `xiaohongshu-finance-0.5.0+codex.20260823170757.zip`，必须与同名 `.sha256` sidecar 一起核验。 |
...
- 当前版本实施了整体性能优化，但没有减少普通报销 Gate 1/Gate 2、发放 fresh audit、TOCTOU、fsync、原子发布、恢复或最终复核。正式 A/B 使用 11 对冷样本、11 对热样本和每组 2 次 warm-up；20% 是信息性目标，实际提升低于 20%，验收依据仍是无尾部回退、内存不明显恶化和输出完全等价。
